...
import { productsApi, formatRM, toPrice, type Product, type ProductOption } from '../../src/api/products';
        ...
        <View className="px-5 pt-5">
          ...
          {product.fruit_info && product.fruit_info.length > 0 && (
            <View className="mt-4 p-4 rounded-2xl" style={{ backgroundColor: isDark ? '#2a3320' : '#e8f5e0' }}>
              ...
              <Text className="text-brand-600 text-xs mt-2 leading-4">💚 我们坚持现采现榨，不用浓缩果汁</Text>
            </View>
          )}
        </View>
        ...
        <View className="flex-row items-center">
          ...
          <Pressable
            onPress={async () => {
              ...
              await useCartStore.getState().add({ cart_id: `${product.id}__${Object.values(optionsRecord).join('_')}`, product_id: product.id as any, product_name: product.name_cn, unit_price: unitPrice, quantity, options: optionsRecord, options_label: labels.join(' / '), image_url: product.image_url ?? undefined });
              router.back();
            }}
